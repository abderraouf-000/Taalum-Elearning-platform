function create() {
    ...
    let prev = document.querySelector('.btnleft');
    ...
    function emptyCalendar() {
        document.querySelectorAll('.tablebody .big-calendar li').forEach(function (ele) {
    ...
    function loadCalendar(currmonth, curryear) {
        ...
        const currentDate = document.querySelector('.tablebody .big-currentdate h2')
        ...
        for (let i = firstdatemonth; i <= lastdatemonth; i++) {
            ...
            let ele = document.querySelector(`.tablebody li[data-num-${days[j]}="${curline}"]`);
    ...
    let clickedDateval;
    document.querySelectorAll('.tablebody li:not(.daysname)').forEach(function (ele) {
        if (true) {
            ele.addEventListener('click', e => {
                clickedDateval = document.querySelector('.tablebody .big-currentdate h2').textContent.split(" ")[0] + " " + ele.textContent + " " + document.querySelector('.big-currentdate h2').textContent.split(" ")[1];
